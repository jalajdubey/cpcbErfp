...
return new class extends Migration
{
    ...
    {
        Schema::create('industry_master_data', function (Blueprint $table) {
            $table->id();
            $table->string('batch_reference');
            $table->string('insured_company_id')->unique();
            $table->string('name_of_insured_owner');
            $table->string('business_type');
            $table->string('address');
            $table->string('territorial_limits_district');
            $table->string('territorial_limits_state');
            $table->decimal('annual_turnover_cr',10,2);
            $table->decimal('paid_up_capital_cr',10,2);
    ...
    {
        Schema::dropIfExists('industry_master_data');
    }
};
