...
function shouldDrawCaption(rect, options) {
  ...
  return w - padding > min2 && h - padding > min2;
}
...
var getX = (rect, w) => rect.rtl ? rect.x + rect.iw - w : rect.x + rect._ix;
...
function squarify(values, rectangle, keys = [], grp, lvl, gsum) {
  ...
  for (i = 0; i < n; ++i) {
    o = { value: val(i), groupSum: gsum, _data: values[tmp[i]._idx], level: void 0, group: void 0 };
    if (grp) {
      o.level = lvl;
      o.group = gval(i);
      const tmpRef = tmp[i];
...
function buildData(tree, dataset, keys, mainRect) {
  ...
  function recur(treeElements, gidx, rect, parent, gs) {
    ...
    const gdata = group(treeElements, g, keys, treeLeafKey, pg, parent, groups.filter((item, index2) => index2 <= gidx));
